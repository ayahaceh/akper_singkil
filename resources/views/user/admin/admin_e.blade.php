...
@section('container')
    <div class="row d-flex justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('user.admin.update', ['id' => $user->id]) }}" method="post"
                        ...
                        <x-select-comp label="User akses" name="role" required theme="horizontal"
                            ...
                            <option value="{{ USER_ROLE_SUPER_ADMIN }}" @if (old('role', $user->role) == USER_ROLE_SUPER_ADMIN) selected @endif>
                                Super Admin
                            </option>
                            <option value="{{ USER_ROLE_ADMIN }}" @if (old('role', $user->role) == USER_ROLE_ADMIN) selected @endif>
                                Admin
                            </option>
                            <option value="{{ USER_ROLE_USER }}" @if (old('role', $user->role) == USER_ROLE_USER) selected @endif>
                                User
                            </option>
                        </x-select-comp>

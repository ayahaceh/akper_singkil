    ...
    <section class="horizontal-wizard">
        <div class="bs-stepper horizontal-wizard-example">
            <div class="card-body">
                ...
                <form action="{{ route('publikasi.jurnal.update', ['id' => $data->id]) }}" method="post"
                    ...
                    <x-select-comp label="Koleksi" name="koleksi" id="koleksi" required theme="horizontal">
                        @if (old('koleksi', $data->koleksi_jurnal_id))
                            @php
                                $koleksi = \App\Models\Jurnal\KoleksiJurnalModel::where('id', old('koleksi', $data->koleksi_jurnal_id))->get();
                            @endphp
                    ...
                    <x-select-comp label="Pengarang" name="pengarang[]" id="pengarang" required multiple theme="horizontal">
                        @if (old('pengarang', explode(', ', $data->pengarang)))
                            @foreach (old('pengarang', explode(', ', $data->pengarang)) as $pengarang)
                    ...
                    <x-input-comp label="Penerbit" type="text" name="penerbit" value="{{ $data->penerbit }}"
                        theme="horizontal" required />

                    <x-input-comp label="Tahun Terbit" type="text" maxlength="4" name="tahun_terbit"
                        value="{{ $data->tahun_terbit }}" theme="horizontal" required />

                    <x-select-comp label="Keyword" name="keyword[]" id="keyword" multiple theme="horizontal">
                        @if (old('keyword', explode(', ', $data->keyword)))
                            @foreach (old('keyword', explode(', ', $data->keyword)) as $keyword)
